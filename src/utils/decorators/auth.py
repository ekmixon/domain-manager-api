...
class RequestAuth:
    ...
    def validate(self):
        ...
        if not COGNTIO_ENABLED:
            g.is_admin = True
            return True
        return bool(self.check_cognito_jwt(request))
    ...
    def get_jwt_token(self, request):
        ...
        if not auth or str(auth[0].lower()) != "bearer":
            return None
        return None if len(auth) != 2 else auth[1]
    ...
    def check_admin_status(self):
        ...
        if not COGNTIO_ENABLED:
            return True
        return True if COGNITO_DEFAULT_ADMIN else "admin" in self.groups
...
def auth_required(view):
    ...
    def decorated(*args, **kwargs):
        ...
        if auth.validate():
            ...
            g.is_admin = bool(auth.check_admin_status())
